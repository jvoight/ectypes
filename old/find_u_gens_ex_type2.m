...
for h in pol_seq do
  ...
  S := BoundedHeightElements(1*OE, 400);
  S := Remove([ s : s in S ], 1);
  ...
  for m in [20..26] do
    ...
    G, toG := UR2/HR2;
    S2 := [ (R2!s)@@mR2 : s in S1 ];
    S1 := [ S1[l] : l in [1..#S2] | S2[l] in HR2 ];
    S2 := [ s : s in S2 | s in HR2 ];
    S1 := [ [ S1[n] : n in [1..#S1] | Order(S2[n]) eq Order(HR2.l) ] : l in [1..Ngens(HR2)] ];
    S2 := [ [ S2[n] : n in [1..#S2] | Order(S2[n]) eq Order(HR2.l) ] : l in [1..Ngens(HR2)] ];
    S3 := [ ];
    for l := 1 to Ngens(HR2) do
      S0 := [ ];
      gens := [ ];
      G1 := sub<HR2 | gens>;
      for n in [1..#S2[l]] do
    ...
    for g4 in S3[4], g3 in S3[3], g2 in S3[2], g1 in S3[1] do
      gens := [ g1, g2, g3, g4 ];
      gensG := [ (R2!s)@@mR2 : s in gens ];
      SG := {@ sub<HR2 | u> : u in gensG @};
      SS2 := Subsets({1..#SG}, 2);
      SS2 := [ [ u : u in s2 ] : s2 in SS2 ];
      int_count := &*[ #(SG[m[1]] meet SG[m[2]]) : m in SS2 ];
      G1 := sub<HR2 | gensG>;
      if int_count eq 1 and G1 eq HR2 then
        Append(~SS1, gens);
      end if;
  ...
  SS := {@ s : s in SS @};
  SS := [ { s[l] : s in ss, ss in SS } : l in [1..4] ];
